...
let strNum3 = enmascarar(strNum2);
console.log(strNum3);
...
function calcHoras(mins) {
	...
	return `Horas: ${horas}` + (mins > 0 ? `Minutos : ${mins}` : '');
}
...
console.log(isWeekend(new Date('August 19, 1975 23:15:30')));
console.log(isWeekend(new Date('August 25, 1975 23:15:30')));
...
function numDaysPassed(datePassed) {
	...
	console.log('value of year ' + year.valueOf());
	...
	console.log(
		`Pasaron : ${(dayPass / (1000 * 60 * 60 * 24)).toFixed(
			0
		)} días dende o comezo de ${year.getFullYear()} ate o ${
			datePassed.getMonth() + 1
		} ${datePassed.getDate()}`
	);
}
...
console.log(froitas.splice(froitas.indexOf('kiwis'), 1, 'cereixas', 'peras'));
...
console.log(
	eufemismos('testán langrán bos días pillabán fervellasverzas lacazan')
);
